...
Here follows a simple example of turning a gen_asm into a `m:gen_statem`.
The example comes from the previous User's Guide for `gen_asm`
...
-ifdef(BEFORE_REWRITE).
start_link(Code) ->
    gen_asm:start_link({local, ?NAME}, ?MODULE, Code, []).
-else.
start_link(Code) ->
...
-ifdef(BEFORE_REWRITE).
button(Digit) ->
    gen_asm:send_event(?NAME, {button, Digit}).
-else.
button(Digit) ->
...
-ifdef(BEFORE_REWRITE).
stop() ->
    gen_asm:sync_send_all_state_event(?NAME, stop).
-else.
stop() ->
...
%% gen_asm. There is also handle_event mode which is
...
%% and not behaviour callbacks as in gen_asm
-endif.
...
A generic finite state machine process (`gen_asm`) implemented
...
A `gen_asm` process assumes all specific parts to be located
...
``` text
gen_asm module                    Callback module
--------------                    ---------------
gen_asm:start
gen_asm:start_link                -----> Module:init/1

gen_asm:stop                      -----> Module:terminate/3

gen_asm:send_event                -----> Module:StateName/2

gen_asm:send_all_state_event      -----> Module:handle_event/3

gen_asm:sync_send_event           -----> Module:StateName/3

gen_asm:sync_send_all_state_event -----> Module:handle_sync_event/4
...
the `gen_asm` process terminates.

A `gen_asm` process handles system messages as described
in [sys(3)](`m:sys`).  The sys module can be used for
debugging a `gen_asm` process.

Notice that a `gen_asm` process does not trap exit signals automatically,
this must be explicitly initiated in the callback module.

Unless otherwise stated, all functions in this module fail
if the specified `gen_asm` process does not exist
or if bad arguments are specified.

The gen_asm process can go into hibernation (see `erlang:hibernate/3`)
...
for the functions to be exported from a `gen_asm` callback module.
...
%%%
%%%
%%% The work flow (of the asm) can be described as follows:
%%%
%%%   User module                           asm
%%%   -----------                          -------
%%%     start              ----->             start
...
Whenever a `gen_asm` process is started using
...
the initial [*state data*](#state-data) of the `gen_asm` process.
...
Whenever a `gen_asm` process receives an event sent using `send_event/2`,
...
There is to be one instance of this function
for each possible [*state name*](#state-name).
Whenever a `gen_asm` process receives an event sent using
...
- If `{reply, Reply, NextStateName, NewStateData}`,
  ...
  [`sync_send_event/2,3`](`sync_send_event/3`).
  The `gen_asm` process then continues executing
...
- If `{next_state, NextStateName, NewStateData}`,
  ...
  the `gen_asm` process continues executing in `NextStateName`
...
- If the function returns `{stop, Reason, Reply, NewStateData}`,
  ...
  explicitly using `reply/2`.  The `gen_asm` process then calls
  [`Module:terminate(Reason, StateName, NewStateData)`](`c:terminate/3`)
  and terminates.
""".
...
This function is called by a `gen_asm` process when it receives
...
`Reason` depends on why the `gen_asm` process is terminating.
...
If the `gen_asm` process is part of a supervision tree
...
- The gen_asm process has been set to trap exit signals.
...
Even if the gen_asm process is **not** part of a supervision tree,
this function is called if it receives an `'EXIT'` message
from its parent. `Reason` is the same as in the `'EXIT'` message.

Otherwise, the gen_asm process terminates immediately.

Notice that for any other reason than `normal`, `shutdown`,
or `{shutdown, Term}` the `gen_asm` process is assumed to terminate
...
This function is called by a `gen_asm` process when it is to update
...
the current `gen_asm` process status.
...
> The `gen_asm` module provides a default implementation
...
- One of [`sys:get_status/1,2`](`sys:get_status/1`) is invoked to get
  the `gen_asm` status. `Opt` is set to the atom `normal` for this case.
- The `gen_asm` process terminates abnormally and logs an error.
  `Opt` is set to the atom terminate for this case.

This function is useful for changing the form and appearance
of the `gen_asm` status for these cases.  A callback module
...
describing the current status of the `gen_asm` process.
...
of the current state and status of the `gen_asm` process.
...
relevant details of the `gen_asm` [*state data*](#state-data).
...
-doc """
[FSM name](#asm-name) specification:
`local`, `global`, or `via` registered.

To be used when starting a `gen_asm`. See `start_link/4`.
""".
...
-doc """
[FSM reference](#asm-ref) `t:pid/0` or registered `t:fsm_name/0`.
...
%%%    Mod  ::= atom(), callback module implementing the 'real' asm
...
%%% ---------------------------------------------------
-doc """
Create a standalone `gen_asm` process, not registered.
...
-doc """
Create a standalone `gen_asm` process.
...
-doc """
Create a `gen_asm` process in a supervision tree, not registered.
...
-doc """
Create a `gen_asm` process in a supervision tree.
...
For example, it ensures that the `gen_asm` process
is linked to the supervisor.

The `gen_asm` process calls [`Module:init/1`](`c:init/1`) to initialize.
...
If option **`{timeout, Time}`** is present, the `gen_asm` process
...
to spawn the `gen_asm` process; see `spawn_opt/2`.

> #### Note {: .info }
> Using spawn option `monitor` is not allowed, it causes
> the function to fail with reason `badarg`.

If the `gen_asm` process is successfully created and initialized,
the function returns `{ok, Pid}`, where `Pid` is the pid
of the `gen_asm` process.  If a process with the specified `FsmName`
...
and waits for it to terminate.  The `gen_asm` process calls
...
Sends `Event` to the `FsmRef` of the `gen_asm` process
and returns `ok` immediately.  The `gen_asm` process calls
...
- The `t:pid/0`
- `Name`, if the `gen_asm` process is locally registered
- `{Name, Node}`, if the `gen_asm` process is locally registered
  at another node
- `{global, GlobalName}`, if the `gen_asm` process is globally registered
- `{via, Module, ViaName}`, if the `gen_asm` process is registered
...
Sends an event to the `FsmRef` of the `gen_asm` process
and waits until a reply arrives or a time-out occurs.
The `gen_asm` process calls [`Module:StateName/3`](`c:'StateName'/3`)
to handle the event, where `'StateName'` is the name
of the current state of the `gen_asm` process.
...
Sends an event asynchronously to the `FsmRef` of the `gen_asm` process
and returns `ok` immediately.  The `gen_asm` process calls
...
Sends an event to the `FsmRef` of the `gen_asm` process and waits
until a reply arrives or a time-out occurs.  The `gen_asm` process calls
...
Sends a time-out event internally in the `gen_asm process`
...
The `gen_asm` process calls [`Module:StateName/2`](`c:'StateName'/2`)
to handle the event, where `'StateName'` is the name
of the current state of the `gen_asm` process at the time
...
Sends a delayed event internally in the `gen_asm` process
that calls this function after `Time` milliseconds.
...
the current state of the `gen_asm` process at the time
...
Cancels an internal timer referred by `Ref` in the `gen_asm` process
...
%% enter_loop/4,5,6
%% Makes an existing process into a gen_asm.
%% The calling process will enter the gen_asm receive loop and become a
%% gen_asm process.
...
-doc """
Enter the `gen_asm` receive loop.
...
Makes an existing process into a `gen_asm` process.  Does not return,
instead the calling process enters the `gen_asm` receive loop
and becomes a `gen_asm` process.  The process must have been started
...
is needed than the `gen_asm` behavior provides.
...
handle_msg(Msg, Parent, Name, StateName, StateData, Mod, _Time, HibernateAfterTimeout) -> %No debug here
	...
	{stop, Reason, Reply, NStateData} when From =/= undefined ->
        ...
        {'EXIT', {undef, [{Mod, handle_info, [_,_,_], _}|_]}} ->
            ?LOG_WARNING(#{label=>{gen_asm,no_handle_info},
                           module=>Mod,
                           message=>Msg},
                         #{domain=>[otp],
                           report_cb=>fun gen_asm:format_log/2,
                         error_logger=>
                             #{tag=>warning_msg,
                               report_cb=>fun gen_asm:format_log/1}}),
...
This function can be used by a `gen_asm` process to explicitly send
...
error_info(Reason, Name, From, Msg, StateName, StateData, Debug) ->
    ...
    ?LOG_ERROR(#{label=>{gen_asm,terminate},
                 name=>Name,
                 last_message=>Msg,
...
limit_report(Report, unlimited) ->
    Report;
limit_report(#{label:={gen_asm,terminate},
               last_message:=Msg,
               state_data:=StateData,
               log:=Log,
               reason:=Reason,
...
limit_report(#{label:={gen_asm,no_handle_info},
...
format_log_single(#{label:={gen_asm,terminate},
                    name:=Name,
                    last_message:=Msg,
                    state_name:=StateName,
                    state_data:=StateData,
...
format_log_single(#{label:={gen_asm,no_handle_info},
                    module:=Mod,
                    message:=Msg},
                  #{single_line:=true,depth:=Depth}=FormatOpts) ->
    P = p(FormatOpts),
...
format_log_multi(#{label:={gen_asm,terminate},
                   name:=Name,
                   last_message:=Msg,
                   state_name:=StateName,
                   state_data:=StateData,
    ...
    {Format++ClientFmt,Args++ClientArgs};
format_log_multi(#{label:={gen_asm,no_handle_info},
                   module:=Mod,
                   message:=Msg},
                 #{depth:=Depth}=FormatOpts) ->
    P = p(FormatOpts),
